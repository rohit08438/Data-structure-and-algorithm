...
# Hard-coded packet with at least 10 elements and two zeros
...
print(f"Memory check (id(packet) == id(front_half)): {id(packet) == id(front_half)} (Expect: False)")


print("\n--- Stage 3: In-Place Correction ---")
middle_index = len(scrambled) // 2

# Insert SYNC-BIT if the middle index is strictly an integer
...
# Test 1: Standard Packet (Original example)
test_1 = [5, 12, 0, 8, 21, 34, 7, 19, 0, 3]
print(f"Test 1 (Standard): {scramble(test_1)}")

# Test 2: Odd-length Packet (11 elements)
test_2 = [1, 2, 0, 4, 5, 6, 7, 8, 0, 10, 11]
